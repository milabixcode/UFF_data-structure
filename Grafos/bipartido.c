...
int pinta_grafo(TGrafo *g)
{

    TPilha *pilha = inicializa();
    g->cor = 1;
    push(pilha, g->id_vertice);
    while (!pilha_vazia(pilha))
    {
        TGrafo *topo = busca_vertice(g, peek(pilha));
        TVizinho *vizinho_atual = topo->prim_vizinho;
        //for(TVizinho* vizinho_atual = topo->prim_vizinho; vizinho_atual; vizinho_atual = vizinho_atual->prox )
        while (vizinho_atual)
        {
            TGrafo *vizinho_do_topo = busca_vertice(g, vizinho_atual->id_vizinho);
            if (vizinho_do_topo->cor != -1)
            {
                if (vizinho_do_topo->cor == topo->cor)
                {
                    return 0;
                }
            }
            else
            {
                if (topo->cor == 0)
                {
                    vizinho_do_topo->cor = 1;
                }
                else
                {
                    vizinho_do_topo->cor = 0;
                }
                push(pilha, vizinho_atual->id_vizinho);
            }
...
{
    TGrafo *vertice_atual = g;
    while (vertice_atual)
    {
        if (vertice_atual->cor == -1)
        {
            int grafo_foi_pintado = pinta_grafo(vertice_atual);
            if (grafo_foi_pintado == 0)
            {
                return 0;
            }
        }
