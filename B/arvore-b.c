...
// gcc arvore-b.c -o arvore-b && cat in.txt | ./arvore-b
...
void insere(TNo *no, int d, int pos, int chave, TNo *pt)
{
    ...
    if (no->m < 2 * d)
    { // Tem espaço no nó, então pode inserir diretamente
        for (int i = no->m; i > pos; i--)
        { // Abre espaço para a inserção
            no->s[i] = no->s[i - 1];
            no->p[i + 1] = no->p[i];
        }
        no->s[pos] = chave;
...
TNo *insere_folha(TNo *raiz, int d, int chave)
{
    if (raiz != NULL)
    {
        ...
        int pos = posicao(chave, no);
        if (pos == no->m || chave != no->s[pos])
        { // chave não existe no nó
            insere(no, d, pos, chave, NULL);
            if (raiz->pai != NULL)
            { // Inserção riou uma nova raiz
                raiz = raiz->pai;
            }
        }
    }
...
void particiona(TNo *P, int d, int pos, int chave, TNo *pt)
{
    ...
    // Processa W
    if (P->pai == NULL)
    { // Caso o nó seja raiz, cria o nó superior
        TNo *W = cria(d);
        W->p[0] = P;
        P->pai = W;
    }
...
{
    TLista *lista_menores = NULL;
    // int n = conta_menores(raiz, k);
    int eh_folha = !raiz->p[0];
    if (eh_folha)
    {
        int pos = posicao(k, raiz);
        for (int i = 0; i < pos; i++)
        {
            lista_menores = insere_ordenado(lista_menores, raiz->s[i]);
        }
        if (pos < raiz->m && raiz->s[pos] < k)
        {
            lista_menores = insere_ordenado(lista_menores, raiz->s[pos]);
        }
        return lista_menores;
    }

    lista_menores = menor_k(raiz->p[0], k);

    // acha a posição onde a chave k deveria estar no nó
    int pos = posicao(k, raiz);

    // checa as chaves lista_filho que k no nó atual
    for (int i = 0; i < pos; i++)
    {
        // Insere a chave na lista de forma ordenada
        lista_menores = insere_ordenado(lista_menores, raiz->s[i]);
        TLista *lista_filho = menor_k(raiz->p[i + 1], k);
        TLista *fim_lista = lista_menores;
        while (fim_lista->prox)
        {
            fim_lista = fim_lista->prox;
        }
        fim_lista->prox = lista_filho;
    }
