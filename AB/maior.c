...
TNoA *maior(TNoA *no) 
{
    ...
    //chamada recursiva para cada um dos lados    
    TNoA* esquerda = maior(no->esq);
    TNoA* direita = maior(no->dir);

    TNoA* maiorNum = no;

    //se tem esquerda: comparo com o maior valor
    if(esquerda){
        if(esquerda->chave > maiorNum->chave){
            maiorNum = esquerda;
        }
    }
    //se tem direita: comparo com o maior numero
    if(direita){
        if(direita->chave > maiorNum->chave){
            maiorNum = direita;
        }
    }
    return maiorNum;
